...
def lin_comb_mat_vec_mult(M, v):
    ...
    for key,value in v.f.items():
        L.append(value*mat2coldict(M)[key])
...
def lin_comb_vec_mat_mult(v, M):
    assert(v.D == M.D[0])
    L=[]
    for key in mat2rowdict(M):
        L.append(v[key]*mat2rowdict(M)[key])
    return sum([v for v in L])
...
def Mv_mat_mat_mult(A, B):
    ...
    t=mat2coldict(B)
    pre={}
    for key,vector in t.items():
        pre[key]=(A*vector)
    return coldict2mat(pre)
