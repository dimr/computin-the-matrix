...
def vector_norm(v):
    return math.sqrt(sum(x**2 for x in v.f.values()))
...
def basis(vlist):
    ...
    '''
    t=orthogonalize(vlist)
    tolerance=1e-20
    for vector in t:
        x=math.pow(vector_norm(vector),2)
        if x<tolerance:
            t.remove(vector)
    return t
...
## Problem 2
def zero_vec(v):
    return all(x==0 for x in list(v.f.values()))
