...
def add(u,v):
    ...
    assert u.D == v.D
    return Vec( u.D, {key:getitem(u,key)+getitem(v,key) for key in u.f.keys()|v.f.keys()})

def dot(u,v):
    "Returns the dot product of the two vectors"
    assert u.D == v.D
    return sum([(getitem(u,key)*getitem(v,key)) for key in u.f.keys()])

def scalar_mul(v, alpha):
    "Returns the scalar-vector product alpha times v"
    return Vec(v.D,{ key:v.f[key]*alpha for key in v.f.keys()})

def neg(v):
    "Returns the negation of a vector"
    return Vec(v.D,{key:-1*v.f[key] for key in v.f.keys()})
