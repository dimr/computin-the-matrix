...
indep_vec_1 = Vec({1,2,3,4}, {1:1})
indep_vec_2 = Vec({1,2,3,4}, {2:1})
indep_vec_3 = Vec({1,2,3,4}, {3:1})
indep_vec_4 = Vec({1,2,3,4}, {1:1,2:1,3:1})
...
def is_superfluous(L, i):
    ...
    if (len(L)==1):
        ...
        vv=new_list[i]
        del new_list[i]
        matA=coldict2mat(new_list)
        solution=solve(matA,vv);
        approximation=matA*solution
        residual=(vv-approximation)*(vv-approximation)
        if(residual<10e-14):
            return True
        else:
            return False
...
def superset_basis(S, L):
    ...
    basis=list(S)
    for i in range(len(L)):
        basis_tmp=list(basis)
        basis_tmp.append(L[i])
        if(is_independent(basis_tmp)):
            basis=list(basis_tmp)
    return basis
...
def exchange(S, A, z):
    ...
    Example:
        ...
        True
    '''
    solution=vec2rep(S, z)
    for i in range(len(S)):
        if((S[i] not in A) and (solution[i]!=0)):
            return S[i]    
