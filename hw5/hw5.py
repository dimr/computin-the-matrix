...
exchange_S1 = [w0,v1,w1]
exchange_S2 = [v0,w0,v2]
...
exchange_2_S1 = [w0,v1,w1]
exchange_2_S2 = [v0,w0,v2]
...
row_space_1 = [Vec({0,1,2},{1:2}),Vec({0,1,2},{1:2,2:1})]
col_space_1 = [Vec({0,1},{0:2,1:2}),Vec({0,1},{0:0,1:1})]

row_space_2 = [Vec({0,1,2,3},{2:1,3:1}), Vec({0,1,2,3},{1:2,2:2}),Vec({0,1,2,3},{0:1,2:4})]
col_space_2 = [Vec({0,1,2},{0:1}),Vec({0,1,2},{2:1}),Vec({0,1,2},{0:4,1:2})]
...
def direct_sum_decompose(U_basis, V_basis, w):
    ...
    lenU = len (U_basis)
    Wu = list2vec ([ v for i, v in W.f.items () if i <  lenU ])
    Wv = list2vec ([ v for i, v in W.f.items () if i >= lenU ])
    u=U*Wu
    v=V*Wv
    return (u,v)
