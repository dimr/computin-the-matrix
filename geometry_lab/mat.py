...
def getitem(M, k):
    ...
    return M.f[ (k[0],k[1]) ] if (k[0],k[1]) in M.f.keys() else 0
...
def add(A, B):
    ...
    assert A.D == B.D
    return Mat( ( A.D[0],A.D[1]) ,{ k: (A[k]+B[k]) for k in A.f.keys() | B.f.keys()})

def scalar_mul(M, alpha):
    "Returns the product of scalar alpha with M" 
    return Mat ( (M.D[0],M.D[1]),{ k:v*alpha for k,v in M.f.items()})

def equal(A, B):
    "Returns true iff A is equal to B"
    assert A.D == B.D
    return all(A.D==B.D and [ A[key]==B[key] for key in B.f.keys()| A.f.keys()])
...
def matrix_matrix_mul(A, B):
    ...
    for row in result.D[0]:
        for col in result.D[1]:
            for k in A.D[1]:
